...
Once we prepare our `min_max` data in a table format, we can use the function `implausible_values` to flag those prescriptions with values outside the plausible range as follows.
...
Running the function `implausible_values()` creates additional columns named `implausible_qty` and `implausible_ndd` with values equals to `TRUE` if the given quantity or ndd is outside the plausible range.
