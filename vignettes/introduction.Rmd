...
The Clinical Practice Research Datalink (CPRD) is a UK database of anonymised primary care electronic health records, and is widely used to study the effectiveness and safety of medications.
However, prescription data from CPRD are often messy, with inherent issues such as missing information on stop dates and medication quantities, as well as prescribing instructions provided as unstructured free-text.
In pharmacoepidemiology studies, the data preparation steps used to determine drug exposure are rarely fully reported, yet assumptions made during this stage can have considerable implications for risk attribution of possible adverse events.

We have previously developed a framework^[is a citation needed here?] for dealing with missing information on stop dates and other issues such as overlapping prescription periods.
...
The current R package, `drugprepCPRD`, builds upon the earlier algorithm and aims to make the framework available to a wider audience through its implementation as free, open-source software.

This vignette describes how to use the `drugprepCPRD` package to transform CPRD drug data contained in a typical `therapy.txt` file into information on individuals’ drug use over time.
...
As described below, the `drugprepCPRD` package extracts the dose frequency and dose number from the text instructions and provide a series of data processing steps with multiple options to define start and stop dates of a given prescription.
The package works at the `prodcode` level to give granularity for each medication.

# The algorithm 

The `drugprepCPRD` is made up of the following functions that must be executed sequentially. 
...
Once the package is installed, we load the `drugprepCPRD` package into the working environment as follows. 
...
data_ndd %>%
  ...
  kable(booktabs = TRUE, row.names = FALSE, position = 'ht',
        caption = 'Sample output from the \\texttt{compute\\_ndd} function with \\texttt{decision} option set to \\texttt{min\\_min}')
```
...
Once we prepare our `min_max` data in a table format, we can use the function `check_implausible`^[name subject to change] to flag those prescriptions with values outside the plausible range as follows.

```{r implausible, echo = 1}
data_plausible <- check_implausible(data_ndd, min_max_dat = min_max_dat)
data_plausible %>%
  dplyr::distinct(ndd, qty, implausible_ndd, implausible_qty) %>%
  kable(booktabs = TRUE, caption = 'Flags for implausible values', linesep = '',
        position = 'ht')
```

Running the function `check_implausible()` creates additional columns named `implausible_qty` and `implausible_ndd` with values equals to `TRUE` if the given quantity or ndd is outside the plausible range.
Example output is given in Table \ref{tab:implausible}.

## Processing the CPRD prescription data 

Once these preliminary pre-processing steps are completed, we can use the main function of `drugprepCPRD`, `run.drugPREP`, to evaluate the 10 decision nodes described above.
This can be done by executing an R command of the following form:

```{r drugprep, echo = 1}
result <- run.drugPREP(data_plausible,
                       decisions = c('1b', '2b1', '3b', '4b1', '5b_6',
                                     '6c', '7a', '8d', '9a', '10b'))
subset(result, select = -text) %>%
  kable(booktabs = TRUE, position = 'ht',
        caption = 'Result of \\texttt{run.drugPREP}') %>%
  kable_styling(latex_options = 'scale_down')
```

The result of running the function `run.drugPREP()` provides the start and stop date (`real_stop`) for each prescription.
Note: the value 10000 in the column `gap_to_next` is given for computational purposes and should not be considered as a value indicating the gap between sequential prescriptions. 
