...
We perform the following steps in series:
...
ggplot(out %>%
         mutate(new_id = reorder(as.factor(row_number()), start_date)) %>%
         distinct(patid, prodcode, start_date, .keep_all = TRUE)) +
...
dec <- make_decisions('ignore',
                      ...
                      'close 15')
dec
drug_prep(example_therapy, plausible_values, decisions = dec)
```
