...
The atomic element of the package should be an `impute` function, which takes four principal inputs:
...
```{r, eval=FALSE}
dataset1[ids_to_replace, ndd := fun(ndd), by = grp]
```

Everything beyond this is simply writing wrappers to handle multiple variables or decisions and pass these arguments to such an expression, as well as to collate the outputs into a dataset for fitting and comparing models.
