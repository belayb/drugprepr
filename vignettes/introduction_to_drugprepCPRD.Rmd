...
Compute ndd
: Extract dose frequency (number of times the prescription to be taken per day) and dose number (e.g., number of tablets to take at a time) from the free text and compute the number of daily dose.

Define implusible values
: Given “Plausible” values (e.g. based on prescribing guidelines and clinical experience), this stage identifies those values outside the plausible values range.

Dec1-Handle implausible qty
...
Dec8-Handle multiple prescriptions
...
Dec9-Handle overlaping prescriptions
...
Dec10-Handle short gaps between prescriptions
